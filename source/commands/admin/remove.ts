...
export default class Forage extends Command {
  ...
  public syntax: string = "[item] [quantity]";
  public description: string = "This is to force remove some item/money!";
...
${Utils.generateSyntax(this)}`);
    ...
    const newQty = value ? value - +qty : 0;
    if (newQty <= 0) {
      delete player.inventory[item.toUpperCase()];
    } else {
      player.inventory[item.toUpperCase()] = newQty;
    }
