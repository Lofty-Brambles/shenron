...
> You can do so <t:${results.availableOn}:R>`);
    ...
    const hasWoodAxe = Object.keys(player.inventory).includes(
      TOOLS.WOOD_HATCHET.TAG,
    );
    const hasMetalAxe = Object.keys(player.inventory).includes(
      TOOLS.METAL_HATCHET.TAG,
    );
...
Please look into the \`${this.client.prefix}craft\` command!`);
    ...
    const caption =
      "gone chopping for lumber, deep inside the jungles of Ark...";
